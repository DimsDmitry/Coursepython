'''Pandas - большая библиотека для работы с данными, возможности которой широки:
1) Чтение и запись файлов
2) Вычисление статистики
3) Вставка, удаление столбцов и строк данных
4) Слияние данных
5) Создание таблиц, их фильтрация, группировка
6) Построение графиков


Типы данных в Pandas:
object — это текст,
float64 — это дробное число,
int64 — это целое число.

Метка = индекс

DataFrame - структура данных, содержащая наборы элементов и их индексы.
Создание так же из списка, как и в series

series - одномерный массив с метками. Структура данных, содержащая элементы и их индексы.
Можно создать его из списка, затем передав в series,
или создать сразу передав в класс Series

'''