...
class Label:
    ...
    def __init__(self, x, y, width, height, color):
        ...
        self.image = None  # Инициализируем image
    ...
    def collidepoint(self, x, y):
        """проверяет клик в конкретной точке"""
...
while True:
    ...
    screen.fill(back)  # Обновляем фон каждый кадр

    if wait == 0:
        wait = 20  # столько кадров надпись будет на одном месте
        click = randint(1, num_cards)  # рандомим номер карточки
        for i in range(num_cards):
            cards[i].main_color = YELLOW if i + 1 == click else (200, 255, 200)  # Изменяем цвет фона карточек
